const actions = [
  {
    label: "Create Images",
    icon: (
      <svg viewBox="0 0 24 24" className="w-5 h-5 fill-twitter-gray">
  ...
  return (
    <div className="flex flex-row gap-2 px-4 pb-3">
      {actions.map((action) => (
        <button
          key={action.label}
          className="flex flex-col items-start gap-2 p-3 rounded-2xl bg-[#1a1a1a] hover:bg-[#222] transition-colors flex-1"
        >
          {action.icon}
          <span className="text-twitter-gray text-[13px] font-medium leading-tight text-left">
            {action.label}
          </span>
        </button>
      ))}
